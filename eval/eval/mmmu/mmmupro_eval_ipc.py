...
def process(line, wrong_img_line, args, tokenizer, image_processor, model_config):
    ...
    if line["image_1"] is None:
        ...
        wimage = wrong_img_line["image_1"].convert('RGB')
...
def eval_model(args):
    ...
    for line, wrong_img_line in tqdm(zip(questions, shuffled_questions), total=len(questions)):
        ...
        input_ids, image_tensor1, image_tensor2, image_tensor3, image_tensor4, image_sizes, prompt = process(line, wrong_img_line, args, tokenizer, image_processor, model.config)
