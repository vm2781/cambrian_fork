...
def process(line, wrong_line, wrong_line_2, args, tokenizer, image_processor, model_config):
    ...
    if wrong_line_2["image_1"] is not None:
    ...
    if wrong_line_2["image_1"] is None:
        image = None
        image_size = None
        image_tensor = None
    else:
        # image = line["image_1"].convert('RGBA')
        image = wrong_line_2["image_1"].convert('RGB')
...
def eval_model(args):
    ...
    for line, wrong_line, wrong_line_2 in tqdm(zip(questions, shuffled_questions, shuffled_questions_2), total=len(questions)):
        ...
        input_ids, image_tensor, image_sizes, prompt, image, qs = process(line, wrong_line, wrong_line_2, args, tokenizer, image_processor, model.config)
        ...
        #         img_dir = "examples/mmmu/rdm_shu" + str(example_num) + ".png"
