...
def process(line, wrong_line, args, tokenizer, image_processor, model_config):
    ...
    if wrong_line["image_1"] is not None:
    ...
    if wrong_line["image_1"] is None:
        image = None
        image_size = None
        image_tensor = None
    else:
        # image = line["image_1"].convert('RGBA')
        image = wrong_line["image_1"].convert('RGB')
        image_size = [image.size]
        image_tensor = process_images([image], image_processor, model_config)

    input_ids = tokenizer_image_token(prompt, tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt').unsqueeze(0).cuda()

    return input_ids, image_tensor, image_size, prompt, image, qs, line["image_1"].convert('RGB')
...
def eval_model(args):
    ...
    for line in tqdm(questions, total=len(questions)):
        ...
        input_ids, image_tensor, image_sizes, prompt, image, qs, right_im = process(line, wrong_line, args, tokenizer, image_processor, model.config)
        ...
        if example_num < 3:
            try:
                ...
                img_dir = "examples/runon/mmmu/txt_" + str(example_num) + "_shuffle.png"
                ...
                img_dir2 = "examples/runon/mmmu/txt_" + str(example_num) + "_real.png"
