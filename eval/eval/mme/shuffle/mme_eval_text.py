...
def eval_model(args):
    ...
    with open(chunk_file, "w") as ans_file:
        ...
        for line, wrong_line in tqdm(zip(questions, shuffled_questions), total=len(questions)):
            idx = idx+1
            if idx<valid_chunk[0] or idx>valid_chunk[1]:
                continue

            input_ids, image_tensor, image_sizes, prompt, img, qs = process(line, wrong_line, args, tokenizer, image_processor, model.config)
            ...
            if example_num < 3:
                img_dir = "examples/mme/txt_shu" + str(example_num) + ".png"
                img.save(img_dir)
