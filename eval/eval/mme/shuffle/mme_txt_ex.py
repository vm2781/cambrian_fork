...
def process(line, wrong_line, args, tokenizer, image_processor, model_config):
    ...
    if wrong_line["image"] is not None:
    ...
    if wrong_line["image"] is None:
        image = None
        image_size = None
        image_tensor = None
    else:
        image = wrong_line["image"].convert('RGB')
        image_size = [image.size]
        image_tensor = process_images([image], image_processor, model_config)

    input_ids = tokenizer_image_token(prompt, tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt').unsqueeze(0).cuda()

    return input_ids, image_tensor, image_size, prompt, qs, image, line["image"].convert('RGB') 
...
def eval_model(args):
    ...
    with open(chunk_file, "w") as ans_file:
        ...
        for line in tqdm(questions, total=len(questions)):
            ...
            input_ids, image_tensor, image_sizes, prompt, qs, image, right_image = process(line, wrong_line, args, tokenizer, image_processor, model.config)
            ...
            if example_num < 3:
                try:
                    ...
                    img_dir = "examples/runon/mme/txt_" + str(example_num) + "_shuffle.png"
                    ...
                    img_dir2 = "examples/runon/mme/txt_" + str(example_num) + "_real.png"
