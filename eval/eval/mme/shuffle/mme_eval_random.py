...
def process(line, wrong_line, wrong_line_2, args, tokenizer, image_processor, model_config):
    ...
    if wrong_line_2["image"] is not None:
    ...
    if wrong_line_2["image"] is None:
        image = None
        image_size = None
        image_tensor = None
    else:
        image = wrong_line_2["image"].convert('RGB')
...
def eval_model(args):
    ...
    with open(chunk_file, "w") as ans_file:
        ...
        for line, wrong_line, wrong_line_2 in tqdm(zip(questions, shuffled_questions, shuffled_questions_2), total=len(questions)):
            ...
            input_ids, image_tensor, image_sizes, prompt, img, qs = process(line, wrong_line, wrong_line_2, args, tokenizer, image_processor, model.config)
            ...
            if example_num < 3:
                img_dir = "examples/mme/rdm_shu" + str(example_num) + ".png"
                img.save(img_dir)
