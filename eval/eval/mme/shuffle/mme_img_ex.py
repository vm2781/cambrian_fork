...
def process(line, wrong_line, args, tokenizer, image_processor, model_config):
    qs = wrong_line["question"] #+ f"\n{args.question_extension}"
    right_qs = line["question"] #+ f"\n{args.question_extension}"

    if line["image"] is not None:
        if model_config.mm_use_im_start_end:
            qs = DEFAULT_IM_START_TOKEN + DEFAULT_IMAGE_TOKEN + DEFAULT_IM_END_TOKEN + '\n' + qs
            right_qs = DEFAULT_IM_START_TOKEN + DEFAULT_IMAGE_TOKEN + DEFAULT_IM_END_TOKEN + '\n' + right_qs
        else:
            qs = DEFAULT_IMAGE_TOKEN + '\n' + qs
            right_qs = DEFAULT_IMAGE_TOKEN + '\n' + right_qs
    ...
    return input_ids, image_tensor, image_size, prompt, qs, image, right_qs
    ...
    with open(chunk_file, "w") as ans_file:
        ...
        for line in tqdm(questions):
            ...
            if line["answer"] == "No":
                ...
                print("We have outlier")
                exit()
        # assert(shuffled_questions != questions)
        for line in tqdm(questions, total=len(questions)):
            ...
            wrong_line = None
            if  (line["answer"] == "Yes"):
                wrong_line = random.choice(yes_lines)
            elif(line["answer"] == "No" ):
                wrong_line = random.choice(no_lines)

            input_ids, image_tensor, image_sizes, prompt, qs, image, right_txt  = process(line, wrong_line, args, tokenizer, image_processor, model.config)
            ...
            if example_num < 3:
                try:
                    ...
                    img_dir = "examples/runon/mme/img_" + str(example_num) + "_shuffle.png"
                    ...
                    caption_text2 = "Text:", right_txt,
                    plt.figtext(0.5, 0.02, caption_text2, wrap=True, horizontalalignment='center', fontsize=12)

                    img_dir2 = "examples/runon/mme/img_" + str(example_num) + "_real.png"
